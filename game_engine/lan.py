...
def play(player, queue, publisher, size, internCom, camera=False):
    clear()
    time.sleep(2)
    ...
    connection = Connect(player, queue, publisher, internCom)

    player1 = LANPlayer(size[0], size[1], 'Player', connection, camera) if player == 'console' else LANAI(size[0], size[1], 'ai', connection)
...
def main(args=None):
    ...
    if len(par) == 3 and (par[-2] == 'console' or par[-2] == 'ai') and (par[-1] == 'A' or par[-1] == 'B'):
        ...
        t2 = threading.Thread(target=play, args=(par[-2], extern_queue, publisher, size, internCom, camera))
